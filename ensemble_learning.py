...
from resnet_binary_classifier import Ensemble
import os
...
SIZE_TRAIN_DATASET, SIZE_VAL_DATASET, BATCH_SIZE, EVALUATE_EVERY = 32, 32, 4, 16 # To train on small dataset
L2_WD = 1e-5
...
MODELS = [
    SAVE_DIR + "train_resnet_sequential_unfreeze_best20200604T175157",
    SAVE_DIR + "train_resnet_sequential_unfreeze_best20200604T175157",
]
...
if device.type == 'cpu':
    for path in MODELS:
        model = torch.load(path, map_location=torch.device('cpu')).resnet101
        models.append(model)
else:
    for path in MODELS:
        model = torch.load(path).cuda().resnet101
        models.append(model)


# TENSORBOARD SETUP
NAME = "ensemble"
DATIME = np.datetime64("now").astype(str).replace('-', '').replace(':', '')
SAVE_MODEL_PATH = SAVE_DIR + NAME + "_best" + DATIME
writer = SummaryWriter(SAVE_DIR + NAME + DATIME)
